...
public class UserDAO {
	...
	public String updateProfile(User user) {
		try {
			Connection conn = DBConnection.getInstance().getConnection();
			PreparedStatement st = conn.prepareStatement("UPDATE user SET first_name = ?, last_name = ?, email = ? WHERE userid = ?;");
			st.setString(1, user.getFirst_name());
			st.setString(2, user.getLast_name());
			st.setString(3, user.getEmail());
	...
	public String updatePassword(User user) {
		try {
			Connection conn = DBConnection.getInstance().getConnection();
			PreparedStatement st = conn.prepareStatement("UPDATE user SET password = ? WHERE userid = ?;");
